...
public class SpielWelt implements Serializable, StringListener {
	...
	// Der Boolean, der angibt, ob sich der Spieler im Gespr�ch befindet.
	...
	private void initKampf(Kampf kampf) {
	    ...
	    for(Entity e : kampf.getKampfGegner())
	    	ausgabe.println(e.getNumGen().getUnbest(0) + e.getName() + " n�hert sich.");
	    ausgabe.println();
	}
	    ...
	    /*for(int i = 0; i < ausgaben.size(); i++) {
	    	if(ausgaben.get(i) != "") {
	    		if(enti.get(i) instanceof Spieler && !abilities.get(i).gueltigeWaffe(((Spieler)enti.get(i)).getWaffenarten())) {
	    			ausgabe.println("Du hast nicht die richtige Waffe f�r diese F�higkeit ausger�stet, du kannst diese F�higkeit nur einsetzen wenn du");
		    		for(byte b : abilities.get(0).getGueltigeWaffen()) {
		    			ausgabe.print(" " + Waffe.getWaffenartNamen(b));
		    		}
		    		ausgabe.print(" ausger�stet hast.");
		    		return;
	    		}
	...
	private void kampfEndet(boolean sieg) {
	    ...
	    if(sieg) {
	    	...
	    	ausgabe.println("Du erh�lst " + xp + " Erfahrungspunkte.\n");
	    	spieler.addXp(xp);
	    	if(!loot.istLeer()) {
	    		ausgabe.println("Die Gegner haben etwas fallen gelassen, du erh�lst ");
	    		...
	    		ausgabe.print("du steckst die Gegenst�nde weg.");
	    	}
	    } else {
	...
	/**
	 * Diese Methode gibt zurueck, ob sich der Spieler im Gespr�ch mit einem NPC befindet.
	...
	/**
	 * Startet ein Gespr�ch.
	 * @param npc Der neue Gespr�chspartner.
	 */
	...
	public void spielerPositionAnzeigen() {
	    ...
	    ausgabe.println("Verf�gbare Ausg�nge:");
	...
	public void sucheUntersuchbaresObjekt(String objektName) {
	    ...
	    if(o != null) {
	    	...
	    	switch(r.nextInt(8)) {
	    		case(0): ausgabe.println("An " + objektName + " ist nichts auff�lig."); break;
	    		...
	    		case(4): ausgabe.println("Es gibt hier auff�ligere Dinge als " + objektName); break;
	...
	public void sucheGegenstand(String gegenstandName) {
	    ...
	    if(s != null) {
	    	...
	    	switch(r.nextInt(6)) {
	    		...
	    		case(3): ausgabe.println(g.getNumGen().getBest(0) + g.getName() + " k�nnte sich als n�tzlich erweisen, deshalb steckst du " + g.getNumGen().getPers(3).toLowerCase() + "ein."); break;
	    		case(4): ausgabe.println("Du nimmst " + g.getNumGen().getBest(3).toLowerCase() + g.getName() + " an dich, da " + g.getNumGen().getPers(0).toLowerCase() + "n�tzlich aussieht."); break;
	    	...
	    	if(loot == null) {
	    		ausgabe.println("Du kannst hier im Moment nichts mehr ernten, komm sp�ter vorbei.");
	    		return;
	    	}
	    	if(loot.length == 0) {
	    	...
	    	ausgabe.print("du nimmst die Gegenst�nde an dich.");
	...
	public void zeigeGegenstandInfosAn(Gegenstand gegenstand) {
	    if(gegenstand == null) {
	    	switch(r.nextInt(5)) {
	    		...
	    		case(1): ausgabe.println("Dir liegen keinerlei Informationen �ber diesen Gegenstand vor."); break;
	    		case(2): ausgabe.println("Dar�ber hast du keine Informationen."); break;
	    		case(3): ausgabe.println("�ber diesen Gegenstand l�sst sich nichts besonderes sagen."); break;
	    		case(4): ausgabe.println("�ber diesen Gegenstand liegen dir keine Informationen vor."); break;
	    	}
	    	return;
	    }
	...
	public void ausruesten(String nameAusruestung) {
	    ...
	    if(spieler.ruesteAus(g)) {
	    	switch(r.nextInt(2)) {
	    		case(0): ausgabe.println("Du hast " + g.getNumGen().getBest(3).toLowerCase() + g.getName() + " ausger�stet."); break;
	    		case(1): ausgabe.println("Du hast den Gegenstand ausger�stet."); break;
	    	}
	    } else {
	    	switch(r.nextInt(3)){
	    		...
	    		case(2): ausgabe.println(nameAusruestung + " kann nicht ausger�stet werden."); break;
	    	}
	    }
	}
	...
	public void ablegen(String nameAusruestung) {
	    ...
	    if(spieler.legeAb(g)) {
	    	switch(r.nextInt(3)) {
	    		...
	    		case(2): ausgabe.println("Du hast " + g.getName() + " zur�ck in dein Inventar gelegt."); break;
	    	}

	    } else {
	    	switch(r.nextInt(3)) {
	        	case(0): ausgabe.println("Du hast diesen Gegenstand nicht ausger�stet"); break;
	        	case(1): ausgabe.println("Du kannst einen Gegenstand, den du nicht ausger�stet hast, nicht ablegen."); break;
	        	case(2): ausgabe.println(g.getName() + " kann nicht abgelegt werden, da du " + g.getNumGen().getPers(3) + "nicht ausger�stet hast."); break;
	    	}
	    }
	}
	    ...
	    if(typ > -1) {
	    	...
	    	if(gegenstaende != null) {
	    		if(gegenstaende.length > 0) {
	    			switch(typ) {
	    				case(Behaelter.FASS): ausgabe.println("Du �ffnest das Fass und suchst darin nach Gegenst�nden. Du findest "); break;
	    				case(Behaelter.KISTE): ausgabe.println("Du �ffnest die Kiste und suchst darin nach Gegenst�nden. Du findest "); break;
	    				case(Behaelter.TRUHE): ausgabe.println("Du �ffnest den Deckel der Truhe und suchst darin nach Gegenst�nden, du findest in der Truhe "); break;
	    			}
	    			...
	    			switch(typ) {
	    				case(Behaelter.FASS): ausgabe.println("Du �ffnest das Fass und suchst darin nach Gegenst�nden, aber das innere des Fass ist leer."); break;
	    				case(Behaelter.KISTE): ausgabe.println("Du �ffnest die Kiste und suchst darin nach Gegenst�nden, aber in der Kiste befindet sich nichts."); break;
	    				case(Behaelter.TRUHE): ausgabe.println("Du �ffnest den Deckel der Truhe und suchst darin nach Gegenst�nden, aber die Truhe scheint schon seit langem leer zu sein."); break;
	    			}
	    		}
	    		return;
	    	}
