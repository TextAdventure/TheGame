...
public class Kombination implements Serializable {
	...
	public static Gegenstand kombiniere(Gegenstand edukt1, Gegenstand edukt2){
	    ...
	    for(Kombination k: getAlleKombinationen(2).toArray(new Kombination[0])){
	    	...
	    	for(Gegenstand g: k.getAlleEdukte()){
	    		if((g == edukt1 | g == edukt2) & erstes) return k.getProdukt().getGegenstand();
	...
	public static Gegenstand kombiniere(Gegenstand edukt1, Gegenstand edukt2, Gegenstand edukt3){
	    ...
	    for(Kombination k: getAlleKombinationen(3).toArray(new Kombination[0])){
	    	...
	    	zweites = false;
	    	for(Gegenstand g: k.getAlleEdukte()){
	    		if((g == edukt1 || g == edukt2 || g == edukt3) && erstes && zweites) return k.getProdukt().getGegenstand();
	...
	// Diese Methode gibt einen Vector zurueck, der alle Gegenstaende beinhaltet, die mit diesem Gegenstand kombinierbar sind. Wenn null uebergeben wird, dann werden alle Gegenstaende zurueckgegeben.
	public static Vector<Gegenstand> istKombinierbarMit(Gegenstand gegenstand){
	    ...
	    if(gegenstand == null){
	    	for(int i = 1; i < 4; i++){
	    		for(Kombination k: getAlleKombinationen(i).toArray(new Kombination[0])){
	    			for(Gegenstand g: k.getAlleEdukte()){
	    				if(!liste.contains(g)) liste.add(g);
	    			}
	    		}
	    	}
	    	return liste;
	    ...
	    for(int i = 1; i < 4; i++){
	    	for(Kombination k: getAlleKombinationen(i).toArray(new Kombination[0])){
	    		for(Gegenstand g: k.getAlleEdukte()){
	    			if(k.istEdukt(gegenstand) && !liste.contains(g) && gegenstand != g) liste.add(g);
	    		}
	    	}
	    }
	    return liste;
	...
	public static Vector<Gegenstand> istKombinierbarMit(Gegenstand gegenstand1, Gegenstand gegenstand2){
	    ...
	    for(Gegenstand g: liste1.toArray(new Gegenstand[0])){
	    	if(liste2.contains(g) && !schnittmenge.contains(g)) schnittmenge.add(g);
	    }
	    return schnittmenge;
	}
	...
	public Stapel getStapel(Gegenstand gegenstand){
	    ...
	    for(Stapel s: zutaten){
	    	if(gegenstand == s.getGegenstand()) return s;
	    }
	    return null;
	}
	...
	// Gibt an, ob der uebergeben Gegenstand ein gueltiges Edukt dieser Kombination ist.
	private boolean istEdukt(Gegenstand gegenstand){
		for(int i = zutaten.length - 1; i > 0; i--){
			if(getEdukt(i).getGegenstand() == gegenstand) return true;
	    }
	    return false;
	}
